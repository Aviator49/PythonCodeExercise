...
def nested_extraction (nesext:list, inpo:int):
    return nesext[inpo][inpo]
...
def beginning_and_end (bae:list):
    return bae[0] == bae[-1]
...
def long_word_in_collection (lwic:list, zwei:str):
    return (zwei in lwic) and len(zwei) > 4